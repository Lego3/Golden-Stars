...
class JuliaView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private var cReal = JuliaMath.presetAt(JuliaMath.DEFAULT_PRESET_INDEX).real
    private var cImag = JuliaMath.presetAt(JuliaMath.DEFAULT_PRESET_INDEX).imag
    ...
    fun setConstant(real: Double, imag: Double) {
        if (real == cReal && imag == cImag) return
        cReal = real
        cImag = imag
        requestFullRender()
    }
    ...
    private suspend fun computeJulia(
        pixels: IntArray,
        renderWidth: Int,
        renderHeight: Int,
        xMin: Double,
